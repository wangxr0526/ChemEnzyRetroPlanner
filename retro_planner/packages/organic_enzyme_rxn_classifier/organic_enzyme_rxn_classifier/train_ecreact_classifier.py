...
class RXNClassificationDataset(Dataset):
    def __init__(self, rxnfps, labels, num_class, ecs2labels) -> None:
        super(RXNClassificationDataset, self).__init__()
        self.rxnfps = rxnfps
        self.labels = labels
        self.fp_dim = rxnfps.size(1)
        self.num_class = num_class
        self.ecs2labels = ecs2labels
...
def load_dataset_with_random_shuffle(dataset_root):
    ...
    with open(os.path.join(dataset_root, 'ecs2labels.json'), 'r') as f:
        ecs2labels = json.load(f)
    num_class = len(ecs2labels)
    dataset_list = []
    for dataset_flag in ['train', 'val', 'test']:
        ...
        labels = torch.tensor(dataset_df['labels'].tolist()).long()
        dataset_cls = RXNClassificationDataset(rxnfps, labels, num_class=num_class, ecs2labels=ecs2labels)
...
if __name__ == '__main__':
    (train_dataset, val_dataset, test_dataset) = load_dataset_with_random_shuffle(dataset_root='./dataset/preprocessed_data/ecreact_classification')
    ...
    train_config = {
        ...
        'batch_size':128,
        'epochs':500,
        'model_dir': './checkpoints/ecreact_classification',
        'ecs2labels':train_dataset.ecs2labels,
