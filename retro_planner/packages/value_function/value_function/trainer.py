...
class Trainer:
    def __init__(self, model, train_data_loader, val_data_loader, n_epochs, lr,
                 save_epoch_int, model_folder, device, tensorboard_witer):
        ...
        self.tensorboard_witer = tensorboard_witer
    ...
    def _pass(self, data, train=True):
        ...
        r_values = self.model(r_fps.view(-1, fp_dim)
                              ).view((batch_size, n_reactants))
    ...
    def _train_epoch(self, epoch):
        ...
        for data in pbar:
            ...
            self.tensorboard_witer.add_scalar('training loss', loss, epoch)
    ...
    def _val_epoch(self, epoch):
        ...
        for data in pbar:
            ...
            losses.append(loss)
            pbar.set_description('[loss: %f]' % (loss))
            self.tensorboard_witer.add_scalar('validation loss', loss, epoch)
