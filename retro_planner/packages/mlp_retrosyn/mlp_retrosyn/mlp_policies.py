...
def load_dat(path, prod_to_rules, template_rules):
    X, y = [], []
    df = pd.read_csv(path, header=None)
    num = len(df)
    # rnx_smiles = list(df['rxn_smiles'])
    retro_templates = df[0].tolist()
    products = df[1].tolist()
    del df
    for i in tqdm(range(num)):
        # rxn = rnx_smiles[i]
        rule = retro_templates[i]
...
def train_mlp(prod_to_rules, template_rule_path, train_path, test_path, fp_dim=2048,
              batch_size=1024,
              lr=0.001,
              epochs=100,
              weight_decay=0,
    ...
    # train_path = '../train_all_dataset/uspto_all/proc_train_cano_smiles_w_tmpl.csv'
    # test_path = '../train_all_dataset/uspto_all/proc_test_cano_smiles_w_tmpl.csv'
    X_train, y_train = load_dat(train_path, prod_to_rules, template_rules)
    X_test, y_test = load_dat(test_path, prod_to_rules, template_rules)
    time_stamp = strftime("%Y_%m_%d_%H:%M:%S", localtime())
...
def train_all(prod_to_rules, template_rule_path, fp_dim=2048,
    ...
    rollout = RolloutPolicyNet(n_rules=len(template_rules), fp_dim=fp_dim, dropout_rate=dropout_rate)
    print('mlp model training...')
    train_path = '../train_all_dataset/uspto_all/proc_all_cano_smiles_w_tmpl.csv'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--template_path', default='../train_all_dataset/uspto_all/templates.dat',
                        type=str, help='Specify the path of the template.train_all_dataset')
    parser.add_argument('--template_rule_path', default='../train_all_dataset/uspto_all/template_rules_1.dat',
